        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-800/80 text-slate-400 uppercase font-semibold text-xs border-b border-slate-700/80 backdrop-blur-md">
              <tr>
                <th className="px-6 py-5 w-[15%]">Tín Hiệu</th>
                <th className="px-6 py-5 w-[25%]">Địa Chỉ IP</th>
                <th className="px-6 py-5 w-[25%]">Vị Trí Lắp Đặt</th>
              ...
              {loading ? (
                ...
                currentItems.map(device => {
                  ...
                  if (!isOnline) {
                    ...
                    dotColor = 'bg-red-500 ring-red-500/20 border border-red-500 animate-[ping_1s_infinite] shadow-[0_0_10px_rgba(239,68,68,0.8)]';
                  } else if (device.incidents && device.incidents.length > 0) {
                    healthText = `LỖI LINH KIỆN: (${device.incidents.length})`;
                    healthColor = 'text-amber-400';
                    dotColor = 'bg-amber-500 ring-amber-500/20 border border-amber-500 animate-[pulse_0.5s_infinite] shadow-[0_0_10px_rgba(245,158,11,0.6)]';
                  }

                  return (
                    <tr key={device.id} className="hover:bg-slate-800/40 transition-colors group">
                      <td className="px-6 py-5">
                        <div className="flex items-center space-x-2 bg-slate-900 w-max px-3 py-1.5 rounded-full border border-slate-800 group-hover:border-slate-700 transition-colors shadow-inner">
                          <span className={`w-2 h-2 rounded-full ${dotColor} ring-2`}></span>
                          <span className={`font-bold text-[10px] tracking-widest uppercase ${healthColor}`}>
                            {healthText}
                          </span>
                        </div>
                      </td>
